...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-176778534-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-176778534-1');
</script>
</head> 
...
vars_to_normalize <- c("TO_SUB_COM", "ALL_READY", "PER_SNO", "TO_RECEP", "PER_FLO", "PER_DRO", "TO_EFFICAC", "HU_SUB_COM",
                       "HU_RESP", "PER_HEA", "PER_TOR", "PER_COL", "TO_RESP", "PER_HUR", "HU_RECEP", "PER_FIR", "HU_OBJ_COM", 
                       "TO_OBJ_COM", "COLD", "FIRE", "FLOOD", "HEAT", "HURR", "SNOW", "TORN", "DROUGHT") # TODO: reorder these
...
cwa_map <- cwa_map %>% 
  mutate_at(vars(all_of(vars_to_normalize)), list(~pnorm(.) * 100)) # convert z-scores to percentile ranks
...
cat <- survey_data %>%
  summarise_all(n_distinct) %>%
  select_if(any_vars((.) <= 11)) # all cat for now

d <- reactive({
  survey_data %>% 
    drop_na(weightfactor, input$exp_group, select_react() %>% pull(var = id)) %>% 
    ...
    mutate(p = round(p * 100, 0), p_low = p_low * 100, p_upp = p_upp * 100) %>% 
    mutate(l = paste0(p, "%")) %>% 
    mutate(resp = factor(!!as.name(select_react() %>% pull(var = id))))
  })

# renderTable({d()}) # use to see raw results in table
...
```{r}
ts_data <- survey_data %>%
  select(data.frame(groups)$groups, weightfactor, follow:risk_fire, alert_und, nws_trust:wx_info8, tor_map_und:svr_watchwarn_und, rec_all:rec_large_group, 
         rec_stream, resp_ignore:resp_large_group, resep_stream, wthr_info_paper:wthr_info_phone)
selectInput(inputId = 'ts_group', label = 'Select a grouping', choices = groups[ !groups == "Year"]) # remove survey year from list
# checkboxInput(inputId = 'scale', label = 'Show full y-scale?', value = FALSE) #TODO: fix this in a future version
```
**Notes.**<br/>
Click on a survey question in the table to the right and a subgroup in the drop-down list above to see the distribution of survey responses to each question by population subgroup.
...
g <- reactive({
  ts_data %>%
    drop_na(weightfactor, input$ts_group, ts_select_react() %>% pull(var = id)) %>% 
    as_survey_design(ids = 1, weights = weightfactor) %>%
    group_by(Year, !!as.name(input$ts_group)) %>%
    summarize(mean = survey_mean(!!as.name(ts_select_react() %>% pull(var = id)), vartype = "ci")) %>% 
    mutate(mean = round(mean, 2))
})

# renderTable({g()}) # use to see raw results in table

renderPlot({
  # ylim <- c(min(ts_data[, ts_select_react() %>% pull(var = id)], na.rm = TRUE),
  #           max(ts_data[, ts_select_react() %>% pull(var = id)], na.rm = TRUE))
  p <- ggplot(g(), aes_string(x = "Year", y = "mean", color = input$ts_group, group = input$ts_group)) +
    geom_point(position = position_dodge(width = 0.1), size = 3) +
    geom_line(position = position_dodge(width = 0.1), size = 1) +
    geom_errorbar(aes(ymin = mean_low, ymax = mean_upp), position = position_dodge(width = 0.1), width = 0, size = 1) +
    labs(x = "Year", y = "Mean", fill = "Group",
         title = variable_data[variable_data$id == ts_select_react() %>% pull(var = id), ]$question,
                  caption = paste0(
           "[",
           variable_data[variable_data$id == ts_select_react() %>% pull(var = id), ]$response_options,
           "]")) +
    # scale_y_continuous(breaks = min(ylim):max(ylim), limits = ylim, expand = expansion(mult = c(0, 0))) +
    theme_classic(base_size = 20) +
    theme(title = element_text(size = 14)) +
    scale_color_viridis(discrete = TRUE) +
    theme(plot.title = element_textbox_simple(margin = margin(0, 0, 22, 0), fill = NA))
  print(p)
  # ps <- p + lims(y = c(min(ts_select_react() %>% pull(var = id), na.rm = TRUE), max(ts_select_react()() %>% pull(var = id), na.rm = TRUE)))
...
div(style = "font-size: 11px",
    selectInput(inputId = 'dem_variable', label = 'Select a demographic or theme', choices = c(
      'Percentage of persons below 150% poverty estimate' = 'POV',
      'Percentage of civilian (age 16+) unemployed' = 'UNEMP',
      'Percentage of housing cost-burdened occupied housing units with annual income less than $75,000' = 'HBURD',
      'Percentage of persons with no high school diploma (age 25+)' = 'NOHSDP',
      'Percentage uninsured in the total civilian noninstitutionalized population' = 'UNINSUR',
      ...
      'Percentage of households without a computer with a broadband Internet subscription estimate' = 'NOINT',
      'Percentage of Black/African American, not Hispanic or Latino persons' = 'AFAM',
      'Percentage of Hispanic or Latino persons' = 'HISP',
      'Percentage of Asian, not Hispanic or Latino persons' = 'ASIAN',
      'Percentage of American Indian or Alaska Native, not Hispanic or Latino persons' = 'AIAN',
      'Percentage of Native Hawaiian or Other Pacific Islander, not Hispanic or Latino persons' = 'NHPI',
      'Percentage of two or more races, not Hispanic or Latino persons' = 'TWOMORE',
      'Percentage of some other race, not Hispanic or Latino persons' = 'OTHERRACE',
      'Percentage of persons in institutionalized group quarters' = 'THEME1',
