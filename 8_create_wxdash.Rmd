...
```{r}
ts_data <- survey_data %>% 
  select(data.frame(groups)$groups, follow:risk_fire, alert_und, nws_trust:wx_info8, tor_map_und:svr_watchwarn_und, rec_all:rec_large_group, rec_stream, resp_ignore:resp_large_group, 
         resep_stream, wthr_info_paper:wthr_info_phone, recep, subj_comp, obj_comp, resp)

selectInput(inputId = 'ts_meas', label = 'Select a measure', choices = names(ts_data %>% select(-data.frame(groups)$groups)))
...
```{r}
renderPlot({
  p1 <- ggplot(ts_data %>% drop_na(input$ts_group, input$ts_meas), aes_string(x = "Year", y = input$ts_meas, color = input$ts_group, group = input$ts_group)) +
...
```{r}
DT::datatable(variable_data %>% filter(id %in% names(ts_data %>% select(follow:und_weather))),
              filter = "top",
              selection = "single",
              rownames = FALSE,
              style = "bootstrap",
