...
library(leaflet)
library(rgdal) # TODO: transition to sf
library(tidyverse)
library(shiny)
library(viridis)
...
#### Estimates by County Warning Area (CWA)
...
output$to_map <- renderLeaflet({
  map$variableplot <- map@data[, input$to_variable]
...
output$map <- renderLeaflet({
  map$variableplot <- map@data[, input$hu_variable]
  p_popup <- with(map@data, paste0("NWS ", City, ", ", ST, " (", CWA,")", "<br>", 
                                   "Percentile rank of the average person in the CWA: ", round(variableplot, 2), "<br>",
                                   "Percentile rank of the CWA: ", round(pnorm(scale(variableplot)) * 100, 2)))
  map <- leaflet(map) %>%
...
miniContentPanel(leafletOutput(outputId = "map"), padding = c(0, 0, 75, 0), scrollable = TRUE)
...
#   fips_map <- fips_map[fips_map$CWA == tract_data()[, 1], ]
...
output$frequency_map <- renderLeaflet({
  col_num <- NA
  col_num <- ifelse(input$event == "PER_HEA", 33, col_num)
  col_num <- ifelse(input$event == "PER_DRO", 37, col_num)
  col_num <- ifelse(input$event == "PER_COL", 30, col_num)
  col_num <- ifelse(input$event == "PER_SNO", 35, col_num)
  col_num <- ifelse(input$event == "PER_TOR", 36, col_num)
  col_num <- ifelse(input$event == "PER_FLO", 32, col_num)
  col_num <- ifelse(input$event == "PER_HUR", 34, col_num)
  col_num <- ifelse(input$event == "PER_FIR", 31, col_num)
...
difference_map_pal <- colorNumeric(palette = difference_map_cols, domain = c(0, 100))

output$difference_map <- renderLeaflet({
  col_num <- NA
  col_num <- ifelse(input$event == "PER_HEA", 33, col_num)
  col_num <- ifelse(input$event == "PER_DRO", 37, col_num)
  col_num <- ifelse(input$event == "PER_COL", 30, col_num)
  ...
  l <- leaflet(map) %>%
    ...
    clearShapes() %>%
    addPolygons(
      fillColor = ~difference_map_pal(freqplot),
      color = "white",
      fillOpacity = 0.9,
      stroke = TRUE,
      smoothFactor = 0.8,
      ...
      layerId = ~CWA,
      highlight = highlightOptions(weight = 5, color = "grey", fillOpacity = 0.9, bringToFront = TRUE)) %>%
    addLegend(position = 'bottomright', pal = difference_map_pal, values = ~freqplot, opacity = 0.7, title = "CWA<br>Percentile<br>Rank")
  })
leafletOutput(outputId = "difference_map")
```
