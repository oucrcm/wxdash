...
variable_data <- fread("variable_data_new.csv")
```
...
<br/>
Contact [Joe Ripberger](mailto:jtr@ou.edu) or [Makenzie Krocak](mailto:mjkrocak@ou.edu) if you have questions about the data or project.
...
miniContentPanel(leafletOutput(outputId = "to_map"), padding = c(0, 0, 75, 0), scrollable = TRUE) %>% withSpinner()
```
...
miniContentPanel(leafletOutput(outputId = "hu_map"), padding = c(0, 0, 75, 0), scrollable = TRUE) %>% withSpinner()
```
...
miniContentPanel(leafletOutput(outputId = "sub_rsk_map"), padding = c(0, 0, 0, 0), scrollable = TRUE) %>% withSpinner()
```
...
miniContentPanel(leafletOutput(outputId = "obj_frq_map"), padding = c(0, 0, 0, 0), scrollable = TRUE) %>% withSpinner()
```
...
miniContentPanel(leafletOutput(outputId = "sub_obj_map"), padding = c(0, 0, 0, 0), scrollable = TRUE) %>% withSpinner()
```
...
<br/>
**Notes.**<br/>
Data from the CDC's Social Vulnerability Index (SVI), which uses this set of 15 US census variables to help officials identify communities that may need support in preparing for hazards or recovering from disasters. For data and more information about the [CDC SVI website](https://www.atsdr.cdc.gov/placeandhealth/svi/index.html).
<br/>
...
miniContentPanel(leafletOutput(outputId = "dem_map"), padding = c(0, 0, 75, 0), scrollable = TRUE)
...
**Data.** All of the survey data and metadata for the Extreme Weather and Society Project are available for download at the [Severe Weather and Society Dataverse](https://dataverse.harvard.edu/dataverse/wxsurvey). 
