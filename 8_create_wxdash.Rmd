...
selectInput(inputId = 'variable', label = 'Select composite measure to explore', choices = c('Tornado Warning Reception' = 'RECEP', 
                                                                                 ...
                                                                                 'Severe Weather Preparedness' = 'READY'))
```
...
output$fips_map <- renderLeaflet({
  ...
  p_popup <- with(map@data, paste0(NAME, ", County ", "(", CWA, ")", "<br>",
...
R7 <- data.frame(
  question = c("In the last year, have you prepared a disaster supply kit with emergency supplies like water, food and medicine that is kept in a designated place in your home?",
               ...
               "In the last year, have you established a specific meeting place to reunite in the event you and your family cannot return home or are evacuated?",
...
output$frequency_map <- renderLeaflet({
  col_num <- NA
  col_num <- ifelse(input$event == "PER_HEA", 30, col_num)
  col_num <- ifelse(input$event == "PER_DRO", 34, col_num)
  col_num <- ifelse(input$event == "PER_COL", 27, col_num)
  col_num <- ifelse(input$event == "PER_SNO", 32, col_num)
  col_num <- ifelse(input$event == "PER_TOR", 33, col_num)
  col_num <- ifelse(input$event == "PER_FLO", 29, col_num)
  col_num <- ifelse(input$event == "PER_HUR", 31, col_num)
  col_num <- ifelse(input$event == "PER_FIR", 28, col_num)
...
renderPlot({
  p1 <- ggplot(d() %>% drop_na(input$exp_group, select_react() %>% pull(var = id)), 
               aes_string(x = "resp", y = "p", fill = input$exp_group, label = "l")) +
  ...
  p2 <- ggplot(survey_data %>% drop_na(input$exp_group, select_react() %>% pull(var = id)), 
               aes_string(y = select_react() %>% pull(var = id), x = input$exp_group, fill = input$exp_group)) +
    geom_boxplot() +
...
ts_data <- survey_data %>%
  select(data.frame(groups)$groups, follow:risk_fire, alert_und, nws_trust:wx_info8, tor_map_und:svr_watchwarn_und, rec_all:rec_large_group, rec_stream,
         resp_ignore:resp_large_group, resep_stream, wthr_info_paper:wthr_info_phone, rq_1:rq_10, recep, subj_comp, obj_comp, resp, ready)
...
output$demo_map <- renderLeaflet({
  ...
  p_popup <- with(fips_map@data, paste0(NAME, ", County ", "(", CWA, ")", "<br>",
