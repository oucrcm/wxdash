...
cwa_map <- cwa_map %>%
  mutate(SCALE_PER_HEA = (pnorm(scale(cwa_map$PER_HEA)) * 100),
         ...
         SCALE_PER_COL = (pnorm(scale(cwa_map$PER_COL)) * 100) - COLD,
         SCALE_PER_SNO = (pnorm(scale(cwa_map$PER_SNO)) * 100) - SNOW,
         SCALE_PER_TOR = (pnorm(scale(cwa_map$PER_TOR)) * 100) - TORN,
         SCALE_PER_FLO = (pnorm(scale(cwa_map$PER_FLO)) * 100) - FLOOD,
         SCALE_PER_HUR = (pnorm(scale(cwa_map$PER_HUR)) * 100) - HURR,
         SCALE_PER_FIR = (pnorm(scale(cwa_map$PER_FIR)) * 100) - FIRE)

renderPlot({
  sub_name <- NA
  sub_name <- ifelse(input$event == "PER_HEA", "PER_HEA", sub_name)
  sub_name <- ifelse(input$event == "PER_DRO", "PER_DRO", sub_name)
  sub_name <- ifelse(input$event == "PER_COL", "PER_COL", sub_name)
  sub_name <- ifelse(input$event == "PER_SNO", "PER_SNO", sub_name)
  sub_name <- ifelse(input$event == "PER_TOR", "PER_TOR", sub_name)
  sub_name <- ifelse(input$event == "PER_FLO", "PER_FLO", sub_name)
  sub_name <- ifelse(input$event == "PER_HUR", "PER_HUR", sub_name)
  sub_name <- ifelse(input$event == "PER_FIR", "PER_FIR", sub_name)
  obj_name <- NA
  ...
  p <- ggplot(data = cwa_map, aes_string(x = obj_name, y = sub_name)) +
  ...
  # p <- ggplot(data = cwa_map, aes_string(x = "DROUGHT", y = sub_rsk)) + geom_point()
  print(p)
  })
```
