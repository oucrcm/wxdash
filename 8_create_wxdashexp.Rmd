...
```{r}
renderPlot({
  p <- ggplot(ts_data %>% drop_na(input$ts_group, select_react() %>% pull(var = id)), 
  aes_string(x = "Year", y = select_react() %>% pull(var = id), color = input$ts_group, group = input$ts_group)) +
    ...
    theme(title = element_text(size = 14)) +
    scale_color_viridis(discrete = TRUE)
  ps <- p + lims(y = c(min(ts_data[, select_react() %>% pull(var = id)], na.rm = TRUE), max(ts_data[, select_react() %>% pull(var = id)], na.rm = TRUE)))
...
DT::datatable(variable_data %>% filter(id %in% names(ts_data)),
              filter = "top",
              selection = "single",
              rownames = FALSE,
              style = "bootstrap",
